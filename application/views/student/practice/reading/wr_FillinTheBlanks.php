            ...
            <div class="col-12 col-lg-12">
                <div class="row">
                    <div class="col-12 col-md-12">
                        ...
                        <form id="fib_wr" class="mb-5"
                            action="<?php echo base_url();?>user/fib_wr/<?php if(isset($getQuestionData) && strlen($getQuestionData[0]->id) > 0){ echo $getQuestionData[0]->id;} ?>"
                        ...
                        <div class="card-body px-10">
                            ...
                            <div class="row mb-2">
                                <div class="col-12">
                                        ...
                                        <p><?php if(isset($getQuestionData) && strlen($getQuestionData[0]->question) > 0){ 
                                        preg_match_all('/{(.*?)}/', $getQuestionData[0]->question, $wrongWords);
                                        $wrongWords = $wrongWords[1];
                                        $text =$getQuestionData[0]->question;
                                        ...
                                        foreach ($options as $optionSet => $option) {
                                            $selected = "";
                                            $select = '<span class="select-blank"><select class="blank" name="selectOptions[]">';
                                            $select .= '<option value=""></option>';
                                            // var_dump($option);echo'<br>';
                                            foreach ($option as $key => $rowOption) {
                                                if(count($response) > 0){
                                                    if($response[$optionSet] == $rowOption){
                                                        $selected = "selected";
                                                    }
                                                }
                                                $select .= '<option value="' . $rowOption . '" '.$selected.'>' . $rowOption . '</option>';
                                                $selected = "";
                                            }
                                            $select .= '</select></span>';
...
<script src="<?php echo base_url() ?>assets/js/app/wr_fib.js"></script>
<script>
...
$('#previous').click(function() {
    location.href = '<?php echo base_url().'user/fib_wr/'.$prev_id; ?>';
});

$('#next').click(function() {
    location.href = '<?php echo base_url().'user/fib_wr/'.$next_id; ?>';
});
...
window.addEventListener('load', function() {
    document.getElementById("fib_wr").reset();
})
