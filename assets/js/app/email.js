    ...
    var csrfHash = $('.csrfToken').val(); // CSRF hash
        $.ajax({
            ...
            success: function(data) {
                ...
                $('.convention').text(data.result.component_score.convention); 
                $('.organization').text(data.result.component_score.organization); 
...
function toogleTips(invoker){
    if(invoker == 'content'){
        ...
        $('.tip-suggestion').html("You do not need to write fancy arguments, which is a very difficult thing when you only have 20 minutes to complete one email. However, if there are two topics in one email question, you have to answer to both of them. Otherwise your content score will be deducted.");
    }
    ...
    if(invoker == 'form'){
        $('.tip-score-guide').html("2 Length is between 200 and 300 words<br>\
            ...
            0 Length is less than 120 or more than 380 words. Email is written in capital letters, contains no punctuation or only consists of bullet points or very short sentences");

        $('.tip-suggestion').html("You MUST get these two marks! You will get a full mark in form simply by writing your email within 200 - 300 words. A full mark will be awarded in Form even if you copy and paste part of your email to meet the 200 words requirement.");
    }
    ...
    if(invoker == 'vocabulary'){
        ...
        $('.tip-suggestion').html("Apply email template and you can easily get a full mark in this dimension.");
    }
    
    if(invoker == 'convention'){
        $('.tip-score-guide').html("");

        $('.tip-suggestion').html("Apply our email template and you can easily get a full mark in this dimension.");
    }
    
    if(invoker == 'organization'){
        $('.tip-score-guide').html("");
